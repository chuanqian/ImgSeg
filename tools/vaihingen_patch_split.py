...
def get_img_mask_padded(image, mask, patch_size, mode):
    ...
    pad_img = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT)(image=img)
    pad_mask = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT)(image=mask)
...
def randomsizedcrop(image, mask):
    ...
    # 确保传递 size 参数来指定裁剪的尺寸
    crop = albu.RandomSizedCrop(
        min_max_height=(int(3 * h // 8), int(h // 2)),
        size=(h, w)  # 确保显式传递裁剪后的目标尺寸
